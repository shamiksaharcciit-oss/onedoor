...
**S6 BUILT, T1–T7** (R053 §5), **shipped in `0.6.0`; `ND-052` COMPLETE**: `TICKETS-ND-052-S6.md`, the epic's last ticket, carrying two
...
changes what a deployment boots with.
**All three ruled by R049 and built.** No semantic pair — *a colour that means two things
means neither* — with prominence ranked by **behaviour**: the silent permit above the loud
denial. A **view that cites**, because a backtest cannot be re-derived without running the
engine and this can; `docs/coverage-derivation.md` is the second half of that ruling, and
writing it exposed an impurity worth reporting (§6 of that doc). And core returned a finding
on §4: **`unobserved` has no enumeration source**, so it is a row only inside a bounded
vocabulary and otherwise the map's footer. The inert refusal is `ND-053`, specced, unbuilt,
landing in its own breaking release.
...
**Four findings.** *(1) Oneview's §3 scope fence does not bind the canvas, and the spec
says so itself* — §1's status line puts product GUIs explicitly out of its scope, so the
canvas inherits §4's tokens, §5's anatomy and §2's law, not the static/read-only delivery
fence; and it inherits `css_block()`'s **raise-never-fall-back** with them (X-6's shape).
*(2) A policy diff is not a verdict, so it may not wear a verdict's colours* —
green-for-added is the most automatic choice a diff UI makes and it would spend the ALLOW/
DENY pair on "this line is new"; the backtest panel's counts ARE verdicts and keep the
semantic pair. Two zones, one test. *(3) A candidate has no home yet* — S1 and S2 both take
`list[Policy]`; the stored rows would be an editing convenience and **`policy_digest`
stays the authority**. *(4) `validate_policy` raises at the first problem* — the canvas
needs a **collecting wrapper over the same function**, never a second validator, and it
must report *problems found* rather than implying completeness (one per rule per pass; set-
level problems are invisible to a per-rule loop).
**Three questions** (§7): the canvas's surface (delivery leans a **separate loopback-bound
Studio server** — the PDP's admin key must not also rewrite the rules decisions are made
...
keep:* computing the preview independently would be a second derivation of a value that
already has an owner, so the preview is produced by ratifying into a scratch store — with
the trap stated, that `_normalized_snapshot` renders the WHOLE policy table, so the scratch
store must hold the candidate MERGED OVER the active set or the previewed number is a
different deployment's hash wearing the right label. *(2) The ceremony must be refusable:*
a UI has a gap between reading and clicking, so ratification is a compare-and-swap against
...
| **`ND-054`** | **Numeric bounds accept the decimal-string form.** Ruled by core's F-B acknowledgment (2026-08-27) on `ESCALATION-20260827-006`: **a conformance defect against AADP §5 and the draft's own worked example**, which carries `"amount_eur": "40.00"` inside `params`. `caps.resolve_cost` accepts `str` and `bounds` does not, so **adding a `numeric` bound changes which wire types an action accepts** — and the refusal pushes callers toward the binary floats the Security Considerations names as an attack surface. Fix is **one shared `numeric_value()`** used by both paths, parsed as `Decimal` and never through `float`. **Widening (denied → permitted), so it lands as the FIRST POST-FREEZE change**, with tests in both directions. `TICKETS-ND-054.md`. | **specced**, held |

| **`ND-055`** | **Policy Studio v2, "The Ledger Room."** Authorized to build by R055 under three constraints: **additive only**, **never gates launch**, and `ND-053`/`ND-054` stay frozen. Binding design authority is `Design_Note_Policy_Studio_V2_2026-08-28.md` (digest `118c61b3…`) plus the approved mockup; deviations are escalations. **P0 (F-G empty state, F-H db trap) SHIPPED** as `0.6.2`. **V1 (shell) BUILT** — tokens vendored and digest-pinned, header, version banner with its three-outcome date, tab bar, four routes, and the R056 §4 seal migration with its two tests inverted in the same commit. **V2 (S1 Policies) BUILT** — reads the snapshot behind the pinned version, three outcomes kept apart, absence-is-denial on every page. **V3 (S4 History) BUILT** with Q4's `--faint` audit and Q6's 404; **Q7 escalated** — the ledger records no caller identity and adding one is a frozen schema change. **V4 (S5 Live state) BUILT** — kill switch read-only with the reason stated, budget bars that do not count held money as spent. **Q7 ruled: a non-secret `key_id`, never a digest of the key.** **V5 (S3 Drafts + the ceremony + Q5's two voices) BUILT** — ratification is a page before it is an action, and the page refuses every claim the engine cannot back. **V6 (re-evaluate under version — the flagship) BUILT** — the engine replays the decision in a scratch database, both versions named, and a version with no snapshot renders `not retrievable` rather than replaying as an empty policy set. **V7 (S2 Editor) BUILT** — two panes over one parsed object, so they cannot disagree; the ND-054 note describes today and hedges toward nothing. **Q8's reporting move made on design grounds; the flake stays OPEN.** **V8 (S6 Verify + the universal law pass) BUILT — ND-055 IS COMPLETE.** The universal pass caught five live pages no law had seen, two shipped fixes stranded by V5, and three tests that had silently stopped testing. Full-arc ledger at the top of `TICKETS-ND-055.md`. **All of V1–V8 ships as `0.7.0` after Sept 12.** The dogfooding walkthrough is prepared (`docs/DOGFOODING.md`), every command run or checked by a test; core arranges the human pass with Shamik before `0.7.0` assembles. **Channel stood down to ready — nothing assembles before Sept 12.** Decomposition and the open questions: `TICKETS-ND-055.md`. Forward 005 (`f3142274…`) pins V8(a)'s test shape as a **sabotage pair** — literal injection and the class route — that must fail against the current implementation first. | **P0 built**, V1–V8 held |

| **`ND-056`** | **Studio authoring paths — three tracks, additive, pre-launch.** Ruled by `Core_Forward_006_Studio_Authoring_Paths_2026-08-30.md` (`f20677b3…`) and `Core_to_Delivery_Response_066_2026-08-30.md` (`16781a0c…`), which ruled the ND-056 proposal (`escalations/PROPOSAL-20260830-ND-056.md`, `1a5de44a…`) and answered Q9–Q14. **T1 BUILT** — upload, and the loader's whole four-stage boot path reachable before boot, reported at the stage that stops; **two lists, never merged**, with every decision-time forecast naming the `CheckId` the engine will record. **T2 BUILT** — draft CRUD, per-rule updates, validation as data, submit-for-ratification, `openapi_url` on and `/docs`+`/redoc` off (Q12); **no approval-by-API**, and the shipped legacy `POST /draft/{id}/ratify` kept for this release with a **witness test** and a **deprecation field**, retiring with the actor-identity work (C1, R066 §1). **T3 BUILT** — BYO endpoint, off by default and *absent* rather than broken; six walls, the sixth being the constitution's principle 4 which the directive failed to transcribe (C5, R066 §4). Studio schema **2 → 3** (`state`, `derivation_record_digest`); **no enforcer migration number claimed**, and a test holds `0019`+ unclaimed. Six contradictions reported before any code; five self-caught defects during it. **Ships as `0.7.0`** — the number follows the content (R066 §2) — **gated on Shamik's dogfooding pass**. T3's slip target is `0.7.1` alone, held severable by a structural test. `TICKETS-ND-056.md`. | **T1–T3 built**, held for `0.7.0` |

| **`ND-057`** | **`compensating_command` names an action type that is never checked to exist — at load time or at decision time.** Filed on core's instruction (R086 §2E, Finding 8, dogfooding pass): the field is stored by `policy_loader.py` and resolved against nothing, so a Tier-1/Tier-2 policy can point its reversal at a typo'd or retired action type and load clean — the referent is fiction and nothing notices until an actual undo is attempted and has nowhere to go. `docs/policy-reference.md` said "must name another registered action type," which was never true; corrected in place (superseding the digest recorded there) as part of the same finding, and this ticket is that correction's other half: the doc now says what the code does, and this item tracks making the code say what the doc used to claim. **Not built here** — R086 §3 names this explicitly as out of scope for the dogfooding-pass fixes. **Sequenced behind the `ND-056` T3-for-`0.7.1` queue** — nothing before T3's slip target lands. |
...
| `0.7.0` | **ND-055** (V1–V8, the Ledger Room) + **ND-056** (T1, T2, T3) — additive only; the number follows the content (R066 §2), and the release is gated on the human dogfooding pass, not the calendar | Studio only; no AADP status change. |
