...
**None blocking.** S5's three questions are ruled by R051 and built to; nothing is
waiting on core.

**S5 is built, T1–T6.** `onedoor/templates/payments/` ships concrete fail-closed policies
with **no placeholders**; `PACK_DIGEST` is generated by `python -m scripts.pack_digest`
and never typed; the meaning identity remains the **cited** `policy_digest`; adoption goes
through the ratification ceremony, so `candidate_digest` equals the pack's `policy_digest`
by construction and lineage is recoverable by recomputation rather than a stored pointer;
and the boundary statement names sanctions screening, KYC, chargebacks, multi-currency
settlement and regulatory reporting **as absent, by name**.

**Three defects the ticket's own tests caught, reported rather than quietly fixed.**

1. **The wheel shipped no template data.** `include = ["onedoor*"]` ships Python modules;
   the pack's `.yaml` and `.md` needed a `package-data` entry. **The `0.3.0` defect
   exactly** — a wheel that shipped no migrations — caught this time by a test written
   before the build rather than by a user's first query.
2. **The webhook rule was semantically inverted.** `param_effects` `hosts` names the
   *sensitive* destinations and applies its effects **when the target matches**; the pack
   had been written as an allowlist meaning *anything not on this list is suspect*, which
   the engine has no rule shape for. Caught **because the behavioural tests assert
   verdicts rather than non-errors** — R051's requirement earning itself. The pack now
   states the rule's direction out loud and demonstrates U4 properly: a shortened URL that
   might be the partner gets the same floor, so the control cannot be dodged with `t.co`.
3. **A gate test's message outran its evidence.** `test_each_pattern_matches_its_own_gates_real_output`
   reported *"the gate can never pass"* whenever the repo simply did not pass that gate —
   sending a reader hunting for a contract bug that was not there. It now holds the three
   outcomes apart: a red gate makes the contract **unverifiable**, and it says so and
   names the real failure.

**And one process defect of delivery's own**, worth recording because it hid a stale
claim: a ledger script used `.replace()` **without asserting the anchor was present** for
two edits. A `.replace()` that matches nothing is a silent no-op, so `CONFORMANCE.md` went
on claiming *"as of Response 048"* long after 050 had landed, and it surfaced only when a
later script crashed looking for the string it believed it had written. **A substitution
that cannot fail is not an edit, it is a wish** — every edit in the S5 scripts asserts its
anchor.

**Core → delivery:** none, as of Response 051 (2026-08-27). `0.5.0` is **published** and
`ND-052` S1–S5 are built; **S6, the proposer, is last** and lands on a proven spine. `ND-001` is built (C1–C5),
